...
一発で作成した。大丈夫かな。ユーザーは設計書を見ないテイだが、みてみる。
nginx任意とかある。過剰。自動チェック効くかチェックしてと頼んでみる
怪しいけど一応それなりにできたっぽい。チェックするの面倒なので実装させてみる。
実装は自前じゃない方が品質良さそうなので、codeモードで実装してもらう
「要件定義書と設計書を見て実装して」
小分けにしたいというので、小分けに実行してもらう
生成しまくるからどうしてもレート制限に引っかかる。。。
